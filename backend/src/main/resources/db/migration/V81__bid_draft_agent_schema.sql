create table if not exists bid_agent_runs (
    ...
    status varchar(40) not null default 'DRAFTED',
...
create index if not exists idx_bid_agent_runs_project
    on bid_agent_runs (project_id);

create index if not exists idx_bid_agent_runs_tender
    on bid_agent_runs (tender_id);

create index if not exists idx_bid_agent_runs_status
...
create index if not exists idx_bid_agent_artifacts_run
    on bid_agent_artifacts (run_id);

create index if not exists idx_bid_agent_artifacts_type
    on bid_agent_artifacts (artifact_type);

create index if not exists idx_bid_agent_artifacts_status
