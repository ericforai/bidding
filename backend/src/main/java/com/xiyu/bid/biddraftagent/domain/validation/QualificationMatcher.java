...
public class QualificationMatcher {
  ...
  /** 每个资质名称对应一个预编译的词边界 Pattern（仅用于 length <= 5 的短名称），避免在循环内重复编译。 ConcurrentHashMap 以应对可能的并发调用场景。 */
  ...
  /**
   * 智能匹配： - 长名称（length > 5，如"ISO9001质量管理体系"）：使用不区分大小写的子串匹配，误匹配风险低。 - 短名称（length <=
   * 5，如"ISO"）：如果全部是英文字母（如"ISO"），仅使用词边界正则匹配，防止"ISO"匹配"ISOLATED"。
  ...
  boolean isSmartMatch(String source, String target) {
    ...
    Pattern pattern =
        shortNamePatternCache.computeIfAbsent(
            target,
  ...
  public record QualificationMatchResult(
      List<MatchedQualification> matched, List<String> missing) {}

  public record MatchedQualification(String requirementText, QualificationDTO qualification) {}
}
