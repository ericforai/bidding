...
public class TenderImportTemplateBuilder {
    ...
    private void buildDictionarySheet(Workbook workbook) {
        ...
        String[] columns = {"地区（总部所在地）", "客户类型", "优先级"};
        ...
        int total = Math.max(regions.size(), Math.max(customerTypes.size(), priorities.size()));
    ...
    private int columnWidth(String header) {
        ...
        if (header.contains("标签")) return 7000;
        if (header.contains("标题")) return 9000;
        return 5000;
    }
}
