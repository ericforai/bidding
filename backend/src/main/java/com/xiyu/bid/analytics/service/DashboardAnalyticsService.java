// Input: Analytics repositories (Tender, Project, Task, User)
// Output: Dashboard overview and drill-down analytics data
// Pos: Service/业务层
...
public class DashboardAnalyticsService {
    ...
    public AnalyticsDrillDownResponseDTO getRevenueDrillDown(String status, LocalDate startDate, LocalDate endDate, Integer page, Integer size) {
    ...
    public AnalyticsDrillDownResponseDTO getWinRateDrillDown(String outcome, LocalDate startDate, LocalDate endDate, Integer page, Integer size) {
    ...
    public AnalyticsDrillDownResponseDTO getTeamDrillDown(String role, LocalDate startDate, LocalDate endDate, Integer page, Integer size) {
        ...
        for (Project project : filteredProjects) {
            ...
            accumulateTeamAggregate(aggregates, project.getManagerId(), amount, won, active, true);
            for (Long memberId : Optional.ofNullable(project.getTeamMembers()).orElse(List.of())) {
        ...
        List<AnalyticsDrillDownRowDTO> baseRows = aggregates.entrySet().stream()
                ...
                .sorted((left, right) -> {
                    ...
                    return Comparator.nullsLast(Long::compareTo)
                            .compare(right.getWonCount(), left.getWonCount());
                })
                .sorted((left, right) -> {
                    int scoreCompare = Comparator.nullsLast(Integer::compareTo)
                            .compare(right.getScore(), left.getScore());
    ...
    public AnalyticsDrillDownResponseDTO getProjectDrillDown(String status, LocalDate startDate, LocalDate endDate, Integer page, Integer size) {
        Map<Long, Tender> tenderById = tenderRepository.findAll().stream()
                .collect(Collectors.toMap(Tender::getId, Function.identity()));
        Map<Long, String> userNameMap = getUserNameMap();
    ...
    private AnalyticsDrillDownResponseDTO buildDrillDownResponse(
            String metricKey,
            String metricLabel,
            LocalDate startDate,
            LocalDate endDate,
