...
class ProjectClosureControllerTest {
    ...
    @Test
    void submit_returnedWithoutEvidence_returns422() throws Exception {
        // depositReturned=true 但未提供 date/evidence → controller 层早期拒绝 422
        ClosureSubmitRequest req = ClosureSubmitRequest.builder().depositReturned(true).build();
        mockMvc.perform(post("/api/projects/1/closure")
                        .contentType(MediaType.APPLICATION_JSON)
