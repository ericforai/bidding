...
export function buildTemplatePayload(data = {}) {
  return {
    name: data.name,
    category: normalizeTemplateCategory(data.category || 'technical'),
...
function buildTemplateQuery(params = {}) {
  ...
  if (params.category && params.category !== 'all') {
    query.category = normalizeTemplateCategory(params.category)
  }
...
export function filterTemplates(items, params = {}) {
  ...
  return items.filter((item) => {
    ...
    if (activeProductType && item.productType !== activeProductType) {
      return false
    }
    if (activeIndustry && item.industry !== activeIndustry) {
      return false
    }
    if (activeDocumentType && item.documentType !== activeDocumentType) {
      return false
    }
    if (params.name) {
