...
describe('ProjectStageTimeline', () => {
  ...
  it('renders 6 PRD stages in order', async () => {
    projectLifecycleApi.getStage.mockResolvedValue({
      data: { currentStage: 'EVALUATION', completedStages: ['INITIATION', 'DRAFTING'] },
    })
    ...
    expect(titles).toEqual(['立项', '标书编制', '评标', '结果确认', '复盘', '结项'])
  ...
  it('emits stage-click for unlocked stages and blocks locked ones', async () => {
    projectLifecycleApi.getStage.mockResolvedValue({
      data: { currentStage: 'DRAFTING', completedStages: ['INITIATION'] },
    })
    const wrapper = mount(ProjectStageTimeline, {
      props: { projectId: 1 },
      global: { stubs },
    ...
    const steps = wrapper.findAll('.el-step')
    await steps[0].trigger('click') // INITIATION (unlocked)
    await steps[3].trigger('click') // RESULT (locked)

    const emitted = wrapper.emitted('stage-click') || []
    expect(emitted).toHaveLength(1)
    expect(emitted[0][0].code).toBe('INITIATION')
