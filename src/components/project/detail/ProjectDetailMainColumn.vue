<template>
  <el-main class="main-content">
    ...
    <ProjectTaskBoardCard :tasks="ctx.project?.tasks || []" :project-id="ctx.project?.id" :can-manage-project-tasks="ctx.canManageProjectTasks" :is-demo-mode="ctx.isDemoMode" @add-task="ctx.handleAddTask" @reset-tasks="ctx.handleResetTasks" @task-click="ctx.handleTaskClick" @status-change="ctx.handleTaskStatusChange" @generate-tasks="ctx.handleGenerateTasks" @tender-breakdown="ctx.handleOpenTenderBreakdown" @score-draft-decompose="ctx.handleOpenScoreDraftDecompose" @add-deliverable="ctx.handleAddDeliverable" @remove-deliverable="ctx.handleRemoveDeliverable" @submit-to-document="ctx.handleSubmitToDocument" />
