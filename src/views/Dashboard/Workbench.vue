...
async function loadPendingApprovals() {
  try {
    ...
    if (result.success && result.data) {
      // 将API数据转换为组件需要的格式
      pendingApprovals.value = result.data.map(item => ({
        id: item.id,
        title: item.projectName + ' - ' + item.typeName,
        type: item.type,
        department: item.applicantDept,
        time: formatRelativeTime(item.submitTime),
        ...item
      }))
    }
  } catch (error) {
...
/* 投标日历按钮 - 加深文字颜色 */
.selected-event-actions .el-button,
.upcoming-event-side .el-button {
  color: #000000 !important;
}

.selected-event-actions .el-button:hover,
.upcoming-event-side .el-button:hover {
