...
import {
  buildWinProbabilityView, formatTenderDisplayField, formatTenderIndustry,
  getTenderDateTimeParts, safeTenderUrl,
...
export function useBiddingDetailPage() {
  ...
  const winProbabilityView = computed(() =>
    buildWinProbabilityView(scoreForView.value?.totalScore ?? tender.value?.aiScore ?? 0))
  const probabilityRate = computed(() => winProbabilityView.value.rate)
  const advantages = computed(() => {
    const dimensions = scoreForView.value?.dimensionSummaries || []
    return dimensions.filter(d => d.score >= 70).slice(0, 4)
      .map(d => `${d.name}当前得分 ${d.score} 分，可作为投标策略重点。`)
  })
  const suggestions = computed(() => {
    const summary = scoreForView.value?.summary || tender.value?.aiReason
    if (!summary && advantages.value.length === 0) return []
    return [
      { title: '评分建议', type: 'success', content: summary || '请结合高分维度组织投标响应材料。' },
      { title: '跟进提醒', type: 'warning', content: '请核对评分证据与招标文件要求，缺口项应在立项前闭环。' },
    ]
  })

  const normalizeCaseIndustryQuery = (industry) => ({
    政府: 'government', 能源: 'energy', 交通: 'transport', 制造业: 'manufacturing',
    教育: 'education', 医疗: 'healthcare', 互联网: 'internet',
    GOVERNMENT: 'government', ENERGY: 'energy', TRANSPORTATION: 'transport',
  ...
  const handleShare = () => ElMessage.success('分享链接已复制到剪贴板')
  const handleViewCase = (caseId) => router.push({ path: '/knowledge/case/detail', query: { id: caseId } })
  ...
  const loadRelatedCases = async () => {
    ...
    try {
      ...
      const result = await knowledgeApi.cases.getList({
        keyword, industry: normalizeCaseIndustryQuery(tender.value.industry),
        page: 1, pageSize: 4, sort: 'recent',
      })
      relatedCases.value = result?.success && Array.isArray(result.data) ? result.data : []
    } catch { relatedCases.value = [] } finally { relatedCasesLoading.value = false }
  }
  ...
  const handleGenerateMatchScore = async () => {
    ...
    } finally { scoreGenerating.value = false }
  }

  const handleConfigureMatchScore = () => router.push({ path: '/settings', query: { tab: 'bid-match-scoring' } })
  ...
  return {
    showTenderAiSection, tender, isFollowed, matchScore, scoreLoading, scoreGenerating, scoreError,
    matchScoreState, scoreEmptyText, scoreEmptyDescription, regionMeta, industryMeta, deadlineParts,
    winProbabilityView, probabilityRate, advantages, suggestions, relatedCases, relatedCasesLoading,
    getScoreClass, getStatusType, getStatusText, getDeadlineClass,
    handleParticipate, handleFollow, handleShare, handleViewOriginal, handleViewCase,
    loadMatchScore, handleGenerateMatchScore, handleConfigureMatchScore, handleAbandon,
  }
}
