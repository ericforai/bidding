// Input: crawler API, search state, and browser storage
...
import {
  ...
  summarizeCrawlerResult,
} from './helpers.js'

const STORAGE_KEY = 'tenderSourceConfig'

export function useTenderSourceConfig({ crawlerApi, refreshTenderList, searchForm, canSyncExternalSource }) {
  ...
  const testConnection = async () => {
    ...
    try {
      await crawlerApi.trigger({ keyword: searchForm.value?.keyword || '', pageSize: 1 })
  ...
  const syncExternalTenders = async () => {
    ...
    try {
      const response = await crawlerApi.trigger({
        keyword: searchForm.value?.keyword || '',
        pageSize: 20,
      })
      fetchResult.value = summarizeCrawlerResult(response)
