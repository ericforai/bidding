...
Mock 模式下内置以下演示账号，用于功能展示和验收测试：

| 用户名 | 用户 ID | 角色 | 部门 | 用途 |
|--------|---------|------|------|------|
| 小王 | U001 | manager | 华南销售部 | 销售视角演示：标讯跟进、项目创建、任务执行、费用申请 |
| 张经理 | U002 | manager | 投标管理部 | 管理视角演示：项目审批、任务分配、进度监控、数据分析 |
| 李总 | U003 | admin | 管理层 | 管理员视角演示：系统设置、全局数据分析、用户与权限管理 |
| 李工 | U004 | staff | 技术部 | 技术人员视角：技术方案编制、任务执行 |

> 注意：API 模式下使用后端真实用户与鉴权体系，演示账号仅在 Mock 模式下可用。
...
在 API 模式下，以下路由会被自动重定向，不作为用户可访问入口：

| 路由名称 | 重定向目标 | 原因 |
|----------|------------|------|
| CustomerOpportunityCenter | `/bidding` | 属于演示功能，未纳入正式商用品范围 |
