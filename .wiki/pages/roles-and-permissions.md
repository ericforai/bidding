...
前端与部分历史接口仍使用 3 个 Legacy 角色（`ADMIN` / `MANAGER` / `STAFF`）。`RoleProfileCatalog.legacyRoleForCode()` 定义回退规则：
...
|---|---|---|
| `admin`, `auditor` | `all` | 全部数据，跨部门、跨区域 |
| `manager`, `bid_admin` | `dept` | 本部门及下属团队数据 |
| `sales`, `bid_lead`, `task_executor`, `bid_specialist`, `admin_staff`, `staff` | `self` | 仅限个人负责的数据 |
